...
export default function AuthModal() {
  ...
  const { authModal, closeAuthModal, openAuthModal, login, register, oauthSignIn } = context
        ...
        <div className="p-8">
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-gray-200 rounded-xl hover:border-primary-500 hover:bg-primary-50 transition-all duration-200 group"
            >
              <FaGoogle className="text-xl text-red-500" />
              <span className="font-medium text-gray-700 group-hover:text-primary-700">Continue with Google</span>
            </button>
            <button
            ...
            >
              ...
              <span className="font-medium text-gray-700 group-hover:text-primary-700">Continue with GitHub</span>
            </button>
          </div>
          ...
          <form className="space-y-4" onSubmit={submit}>
            {isRegister && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary-500 focus:ring-0 outline-none transition-all"
            ...
            <button
              ...
              className={`w-full py-3 text-white font-semibold rounded-xl shadow-lg transition-all duration-200 bg-gradient-to-r from-primary-600 to-primary-700 hover:shadow-xl transform hover:scale-[1.02] ${isRegister && !isStrong ? 'opacity-50 cursor-not-allowed hover:scale-100' : ''}`}
            >
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            {isRegister ? (
              <span>Already have an account? <button onClick={() => openAuthModal('login')} className="text-primary-600 font-semibold hover:text-primary-700">Sign in</button></span>
            ) : (
              <span>Don't have an account? <button onClick={() => openAuthModal('register')} className="text-primary-600 font-semibold hover:text-primary-700">Create one</button></span>
            )}
          </div>
        </div>
      </div>
