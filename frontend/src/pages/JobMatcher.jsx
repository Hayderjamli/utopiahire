...
export default function JobMatcher(){
  ...
  return (
    <div className="container py-10">
      ...
      <textarea className="w-full min-h-[160px] rounded-xl border-2 border-gray-200 p-3 focus:border-indigo-500" value={text} onChange={(e)=>setText(e.target.value)} placeholder="Paste your CV here..." />
      <button onClick={match} className="mt-4 w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold">Find jobs</button>
        ...
        {jobs.map((j, idx) => (
          <div key={idx} className="bg-white rounded-2xl p-4 border border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <button className="text-sm px-3 py-2 rounded-lg border-2 border-gray-200 hover:border-indigo-500 hover:text-indigo-600">Apply</button>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {j.tags.map(t => <span key={t} className="text-xs px-2 py-1 rounded-full bg-indigo-50 text-indigo-700">{t}</span>)}
            </div>
          </div>
        ))}
      </div>
